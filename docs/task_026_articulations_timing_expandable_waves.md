...
## Priority 4c: Editable Chain Cards and Create Word

Articulation Chain cards are now editable after being added. Each card shows the phoneme name, IPA, family, duration, source badge, gap, and crossfade metadata. The card action row uses large toy-like buttons for **▶ Play**, **📂 Load/Edit**, **⬅ Move Earlier**, **➡ Move Later**, and **🗑 Remove**. Removing a single card updates the chain immediately without a confirmation dialog and preserves the order of the remaining cards. The full-chain clear action is separate, asks for confirmation, and does not delete saved phoneme files.

**Play Chain** remains the raw diagnostic preview: it renders each phoneme item with its saved articulation/source snapshot and concatenates the clips. **Create Word** is the word-building action: it leaves the editable chain intact, renders each item, then post-processes the result into the latest smoothed word render. **Play Word** previews that latest smoothed render, creating one first if needed, and **Export Word** writes the latest render to an audio file with a sidecar `.wave-toy-word.json` metadata file. The sidecar stores chain metadata and source paths only; raw audio is not embedded.

The first-pass word post-processing pipeline uses these defaults: 0 ms default gaps, 12 ms default crossfades, 8 ms boundary smoothing, 5 ms word fade-in, and 8 ms word fade-out. The renderer applies clip edge fades, overlap crossfades, gentle smoothing around detected boundaries, and safe normalization. This is intentionally approximate and educational; it reduces choppiness without claiming realistic speech synthesis.

Boundary crossfade rules inspect adjacent phoneme families. Stop-to-vowel transitions use a short 4 ms crossfade to better preserve bursts. Fricative-to-vowel transitions use 18 ms, nasal-to-vowel uses 22 ms, glide/liquid-to-vowel uses 28 ms, vowel-to-consonant uses 10 ms, and vowel-to-vowel uses 30 ms. Future work can move these rules into a richer Word Timeline with editable per-boundary handles, stronger coarticulation controls, and non-destructive word-level arrangement.
